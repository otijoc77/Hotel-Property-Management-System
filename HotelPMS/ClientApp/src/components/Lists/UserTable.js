		...
		<div className="card">
			<table className="table table-striped mb-0" aria-labelledby="tabelLabel">
				<thead className="table-head">
					<tr>
						<th>Name</th>
						{props.users[0].account && <th>Username</th>}
						<th>Email</th>
						<th>Phone Number</th>
						{props.admin && (
							<>
				...
				<tbody>
					{props.users.map((user) => (
						<tr key={user.id}>
							...
							{user.account && <td>{user.account.username}</td>}
							<td>{user.email}</td>
							...
							{props.admin && loaded && (
								<>
									<td>
										...
										>
											{AccountLevels.map((level) => (
												<>
													<option
														defaultValue={level.label == user.account.level}
														value={level.label}
														key={level.value}
													>
														{level.label}
									...
									<td>
										...
										>
											<option
												defaultValue={user.companyId == null}
												value={""}
												key={0}
											>
												No company
											...
											{companies.map((company) => (
												<>
													<option
														defaultValue={company.id == user.companyId}
														value={company.id}
														key={company.id}
													>
														{company.name}
								...
								<>
									<td>
										<select
											name="type"
											className="form-select w-100"
											value={user.account.serviceType}
											onChange={(e) =>
												updateUserService({
													e,
													user: user,
										...
										>
											{ServiceTypes.map((level) => (
												<>
													<option
														defaultValue={
															level.label == user.account.serviceType
														}
														value={level.label}
														key={level.value}
													>
										...
										<select
											...
											onChange={(e) =>
												updateUserHotelId({
													e,
													user: user,
													hotelId: e.target.value != "" ? e.target.value : null,
												})
											}
										>
											<option
												defaultValue={user.hotelId == null}
												value={""}
												key={0}
											>
												No hotel
											...
											{hotels.map((hotel) => (
												<>
													<option
														defaultValue={hotel.id == user.hotelId}
														value={hotel.id}
														key={hotel.id}
													>
														{hotel.name}
